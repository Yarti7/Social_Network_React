...
describe("ProfileStatus component", () => {
   ...
   test("correct status", () => {
     ...
     const root = component.root;
     let span = root.findByType("span");
     expect(span.children[0]).toBe("yoyoyo");
   });

    test("input be displayed in editMode instead of span", () => {
        const component = create(<ProfileStatus status="yoyoyo" /> );
        const root = component.root;
        let span = root.findByType("span");
    ...
    test("callback should be called", () => {
        ...
        const component = create(<ProfileStatus status="yoyoyo" updateStatus={mockCallback} /> );
        const instance = component.getInstance();
